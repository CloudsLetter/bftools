...
public partial class RuleView : UserControl {
    ...
    private async Task<bool> SetRuleFromCloud(bool refresh) {
        ...
        if (result.IsSuccess) {
            ...
            this.Dispatcher.Invoke(
                new Action(
                    delegate { ListBox_CustomWhites.Items.Clear(); }
                )
            );
            ...
            foreach (Players player in players) {
                this.Dispatcher.Invoke(
                    new Action(
                        delegate { ListBox_CustomWhites.Items.Add(player.PlayerName); }
                    )
                );
            }
        ...
        if (result2.IsSuccess) {
            ...
            this.Dispatcher.Invoke(
                new Action(
                    delegate { ListBox_CustomBlacks.Items.Clear(); }
                )
            );
            ...
            foreach (Players player in players) {
                this.Dispatcher.Invoke(
                    new Action(
                        delegate { ListBox_CustomBlacks.Items.Add(player.PlayerName); }
                    )
                );
            }
        } else {
            ...
            if (Globals.IsCloudMode) {
                if (Globals.ServerId != 0) {
                    if (Globals.LoginPlayerIsAdmin) {
                        ...
                        if (Globals.SystemAutoBalance && Globals.Team2PlayerCount - Globals.Team1PlayerCount <= 1 &&
                            Globals.Team2PlayerCount - Globals.Team1PlayerCount >= 0 || Globals.Team1PlayerCount - Globals.Team2PlayerCount <= 1 &&
                            Globals.Team1PlayerCount - Globals.Team2PlayerCount >= 0) {
...
*/
    ...
    private void SetAndApplyRule() {
        ...
        // 检查队伍1等级限制
        if (Globals.ServerRule_Team1.MinRank >= Globals.ServerRule_Team1.MaxRank && Globals.ServerRule_Team1.MinRank != 0 &&
            Globals.ServerRule_Team1.MaxRank != 0) {
            Globals.IsSetRuleOK = false;
        ...
        // 检查队伍2等级限制
        if (Globals.ServerRule_Team2.MinRank >= Globals.ServerRule_Team2.MaxRank && Globals.ServerRule_Team2.MinRank != 0 &&
            Globals.ServerRule_Team2.MaxRank != 0) {
            Globals.IsSetRuleOK = false;
        ...
        // 检查重开比分设置 
        if (Globals.ServerRule_Team1.ScoreGap >= Globals.ServerRule_Team1.ScoreLimit && Globals.ServerRule_Team1.ScoreLimit != 0 &&
            Globals.ServerRule_Team1.ScoreGap != 0) {
            Globals.IsSetRuleOK = false;
        ...
        for (int i = 0; i < Math.Max(team1, team2); i++) {
            if (i < team1 && i < team2) {
                AddRuleLog($"武器名称 {i + 1}", $"{ClientHelper.GetWeaponChsName(Globals.CustomWeapons_Team1[i])}",
                    $"{ClientHelper.GetWeaponChsName(Globals.CustomWeapons_Team2[i])}");
            } else if (i < team1) {
                AddRuleLog($"武器名称 {i + 1}", $"{ClientHelper.GetWeaponChsName(Globals.CustomWeapons_Team1[i])}");
            } else if (i < team2) {
    ...
    private async void Button_AddNewWhite_Click(object sender, RoutedEventArgs e) {
        if (Globals.IsCloudMode) {
            ...
            var result = await BF1API.AddWhiteList(ServerId: Globals.ServerId.ToString(), Gameid: Globals.GameId.ToString(),
                Guid: Globals.PersistedGameId, PlayerName: name, ServerName: Globals.ServerName);
            if (result.IsSuccess) {
    ...
    private async void Button_AddNewBlack_Click(object sender, RoutedEventArgs e) {
        if (Globals.IsCloudMode) {
            ...
            var result = await CloudApi.AddBlackList(ServerId: Globals.ServerId.ToString(), Gameid: Globals.GameId.ToString(),
                Guid: Globals.PersistedGameId, PlayerName: name, ServerName: Globals.ServerName);
            if (result.IsSuccess) {
    ...
    private void TranslateKeyRules_Preset_Whitespace_Checked(object sender, RoutedEventArgs e) {
        TranslateKeyRules_Helper_ApplyPreset(TranslateKeyData.TranslateKeyFlag.WHITESPACE,
            CheckBox_TranslateKeyRulePresets_Whitespace.IsChecked.Value);
    }

    private void TranslateKeyRules_Preset_FakeEnglish_Checked(object sender, RoutedEventArgs e) {
        TranslateKeyRules_Helper_ApplyPreset(TranslateKeyData.TranslateKeyFlag.FAKE_ENGLISH,
            CheckBox_TranslateKeyRulePresets_FakeEnglish.IsChecked.Value);
    }
    ...
    private void TranslateKeyRules_Helper_ApplyPreset(TranslateKeyData.TranslateKeyFlag flag, bool addOrRemove) {
        var presetList = TranslateKeyData.TranslateKeys
